...
|----|-------|------|-------------|---------------|---------|--------|--------|--------|
| TD-001 | 38 | BUG | exp97 GPU: 0% success, 0 modulations. Root causes: (1) goal_embedding never set → navigator disabled → agent=random, (2) action space mismatch n_actions=7 vs motor=5 → toggle(5) unreachable → DoorKey unsolvable, (3) epsilon=0.3 too low. **Fix applied: stage-38_fix — curiosity/PE-driven action selection.** CPU mechanisms verified (33 tests PASS). GPU re-run needed. | success_rate ≥ 0.10, modulations > 0 | `main` (after merge) | IN_PROGRESS | 2026-04-01 | — |
