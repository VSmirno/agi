...
def main():
    ...
    parser.add_argument("--start-gen", type=int, default=1,
                        help="Start from this generation (1=fresh, 2+=resume without reset)")
    ...
    if args.start_gen == 1:
        # Fresh run: reset promoted_hypotheses.yaml
        promoted_path.write_text("# Auto-generated by TextbookPromoter. Do not edit manually.\nhypotheses: []\n")
        print(f"Reset {promoted_path}")
    else:
        print(f"Resuming from gen {args.start_gen}, keeping existing {promoted_path}")

    print(f"device={device}, dim={model_dim}, locs={n_locations}, n_gens={n_gens}, n_ep={n_episodes}")

    # Synthetic gen1 result when resuming (avg from known gen1 run)
    all_gen_results = []
    if args.start_gen > 1:
        all_gen_results.append({
            "gen": 1,
            "avg_survival": 169.7,  # from completed gen1
            "promoted_in": 0,
            "promoted_out": 1,
            "n_verifiable": 2,
            "curiosity_active_episodes": 16,
            "all_hypotheses": [],
            "episodes": [],
        })

    for gen in range(args.start_gen, n_gens + 1):
