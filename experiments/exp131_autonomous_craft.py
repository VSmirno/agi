    ...
    for step in range(max_steps):
        ...
        if not current_plan or plan_step_idx >= len(current_plan):
            ...
            if action == "babble_do":
                ...
                z_before = vf_b.raw_center_feature if vf_b.raw_center_feature is not None else vf_b.center_feature
        ...
        if near_str == plan_step.target:
            if plan_step.action == "do":
                ...
                if vf.center_feature is not None:
                    grounded = on_action_outcome("do", old_inv, new_inv, vf.raw_center_feature if vf.raw_center_feature is not None else vf.center_feature, store, labeler, encoder=encoder)
                    if grounded:
                        grounding_events.append(f"plan→{grounded}")
                if success:
                    nav_successes += 1
                    # Only advance if next step's prerequisites are met
                    next_idx = plan_step_idx + 1
                    new_inv = dict(info.get("inventory", {}))
                    can_advance = True
                    if next_idx < len(current_plan):
                        next_reqs = current_plan[next_idx].requires
                        if next_reqs:
                            can_advance = all(
                                new_inv.get(r, 0) >= n for r, n in next_reqs.items()
                            )
                    if can_advance:
                        plan_step_idx += 1
                        if verbose:
                            print(f"    [{step}] ADVANCE to step {plan_step_idx}")
                    nav_steps = 0
                ...
                if vf.center_feature is not None:
                    grounded = on_action_outcome(
                        crafter_action, old_inv, new_inv, vf.raw_center_feature if vf.raw_center_feature is not None else vf.center_feature, store, labeler, encoder=encoder)
                    if grounded:
                        grounding_events.append(f"craft→{grounded}")
                        if verbose:
