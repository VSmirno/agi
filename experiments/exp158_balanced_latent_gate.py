"""Class-balanced latent MSE for frozen exp153 action-specific gates."""
...
OBJECTIVE = {
    "target": "frozen encoder next_z",
    "error": "per-transition/member/dimension squared latent error",
    "weight": "fixed weight[action, observed_rgb_change] from exact full corpus",
    "denominator": "ordinary count of valid member-latent elements",
...
def weighted_latent_mse(
    ...
    """Fixed-population weighted MSE with the ordinary valid-element denominator."""

    if member_z.ndim != 3 or target_z.shape != member_z.shape[1:]:
        raise ValueError("member_z must be [members,batch,z] with matching target_z")
    batch = member_z.shape[1]
    ...
    count = int(valid.sum()) * member_z.shape[0] * member_z.shape[2]
    if count == 0:
        return member_z.new_zeros(())
    weights = class_weights[actions, changed.long()]
    errors = (member_z - target_z.unsqueeze(0)).square()
    selected = errors * weights[None, :, None]
    return selected.masked_select(valid[None, :, None].expand_as(selected)).sum() / count


class BalancedLatentGateTrainer(CoreTrainer):
    """Train only gate amplitudes with fixed action/event-weighted latent MSE."""
    ...
    def compute_loss(self, batch: SequenceBatch) -> torch.Tensor:
        ...
        for index in range(stop):
            ...
            weighted_sum = weighted_sum + weighted_latent_mse(
                prediction.member_z,
                z[:, index + 1],
                valid,
                actions,
        ...
        return weighted_sum / max(ordinary_count, 1)


def _checkpoint_payload(model, ordered, shuffled, config, manifest, counts, balance):
    payload = frozen._checkpoint_payload(
        model, ordered, shuffled, config, manifest, counts
    )
    payload.update(
        format_version=6,
        latent_parameterization="frozen_exp153_delta_action_specific_gate_balanced_latent",
        balanced_latent_objective=balance,
    )
    return payload
...
def main(argv=None) -> int:
    ...
    command = os.environ.get("EXP158_LAUNCH_COMMAND") or shlex.join(_argv(argv))
    manifest = {
        "argv": _argv(argv),
    ...
    with temporal.ProgressJournal(
        ...
        try:
            ...
            trainer = BalancedLatentGateTrainer(model, config, class_weights)
            matching = {
            ...
            journal.update("balanced_gate_checkpoint", 0, 1)
            checkpoint_path = args.out / "balanced_latent_gate_checkpoint.pt"
            torch.save(
                _checkpoint_payload(
                    model,
                    ordered,
            ...
            journal.update("balanced_gate_checkpoint", 1, 1)
            diagnostics = {}
            ...
            result = {
                "status": "completed",
                "claim": "balanced-latent objective causal intervention",
                ...
                "conclusion": (
                    "Class imbalance in uniform latent MSE was the blocker for learning "
                    "oracle-like gate amplitudes."
                    if transfer_gate
                    else "Balanced latent MSE still failed despite exp156's oracle bound; "
                    "next test direct self-supervised oracle-amplitude targets or richer "
                    "gate inputs, not a coefficient sweep or raw-delta replacement."
                ),
                "artifacts": {
