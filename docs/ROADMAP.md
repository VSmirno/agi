...
**Stage 90R — Viewport-First Local Survival Reset**
- проверить более жёсткую гипотезу: агент должен сначала научиться coherent local behavior
  из текущего viewport, прежде чем усиливать planner/memory stack
- убрать зависимость следующего шага от `near_concept` и global-memory-first policy
- использовать `spatial_map` только как secondary/debug signal, пока не доказана локальная адекватность
