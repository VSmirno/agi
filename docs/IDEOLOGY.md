...
### Категория 4 — Stimuli (стимулы)

**Где живёт:** отдельный модуль `StimuliLayer`, вычисляемый из текущего
состояния агента. Не в world model. Не в textbook. Не в механизме
планирования.

**Что это:** то **почему** агент вообще что-то делает. Стимулы —
это функции `U(state) → score` которые агент применяет к
симулированным состояниям чтобы выбрать план. Три типа:

| Тип | Пример | Форма |
|-----|--------|-------|
| Гомеостатический | food=2 → плохо | дефицит → штраф |
| Информационный | неожиданный исход → хочу | сюрприз → награда |
| Ограничение | health=0 → невозможно | барьер, не градиент |

**Важное разграничение:**

- `avg_len` (выживание в eval) — это **наш** внешний критерий, не цель агента.
- Агент не «хочет выжить». Он избегает death-barrier (ограничение) и
  удовлетворяет гомеостаз. Выживание — эмерджентный результат.
- `total_gain` (wood) — это **Crafter-специфичная** цель, не универсальная.
  Правильная замена: информационный стимул (любопытство) сам тянет к
  ресурсам через сюрприз, без знания о wood.

**Как вычисляется:**

```python
plan_score = Σ stimulus.evaluate(simulated_final_state)

# Примеры стимулов:
U_homeostasis(s) = Σ min(0, s.body[v] - threshold[v])   # дефицит → штраф
U_curiosity(s)   = world_model.expected_surprise(s)      # новизна → награда
U_survival(s)    = -inf if s.body["health"] <= 0 else 0  # барьер
```

**Как обновляется:** форма стимулов стабильна (это architecture-level
решение). Параметры (пороги, веса) могут уточняться из experience.

**Правило:** если хочется добавить поведение через `if` в плане или
через изменение `score_trajectory` — это стимул. Спросить: какой
стимул отсутствует или неправильно вычисляется?

**Локальные survival affordances:** если рядом уже есть объект, действие
над которым по textbook даёт положительную body-delta для базовой витали
(например `do water → drink +5`, `do cow → food +5`), агент может взять
его до наступления критического дефицита. Факт “какой объект что даёт”
остаётся в textbook; механизм только обобщённо распознаёт локально
доступное действие с positive body effect; приоритет немедленной угрозы
сохраняется за emergency/defensive слоями.

**Текущее состояние (Stage 83):** стимулы захардкожены внутри
`score_trajectory` как `(survived, total_gain, min_vital, -steps)`.
`min_vital` использует фейковые body values (всегда 9.0, т.к. `info`
не содержит реальных витален). Это technical debt следующего stage.
...
4. **Это «почему агент действует» или «что происходит в мире»?**
   - Мотивация / оценка состояния → category 4 (stimuli)
   - Знание о мире → category 1 или 3
...
### Принцип 1 — Четыре категории, не одна
...
Типичные смешения:
- eval-метрика (наш внешний критерий) затекает в category 4 (стимулы агента)
- стимул захардкоживается в category 2 (механизм), вместо отдельного слоя
- goal-directed behaviour пишется как if-else в policy вместо
  правильного стимула + world model
...
### Принцип 6 — Система, не агент

Правильный уровень абстракции: **мы — самообучающаяся система,
играющая в Crafter**. Не «агент в среде».

**Что это меняет:**

| Эпизодный агент | Персистентная система |
|---|---|
| Смерть = конец | Смерть = переход + обучающий сигнал |
| Каждый эпизод независим | Эпизоды = обучающие данные системы |
| avg_len = цель | avg_len = метрика прогресса (наша) |
| World model сбрасывается | World model персистентен |
| "Я умер" | "Почему я умер? Что запомнить?" |

**Смерть как обучающий сигнал:**

Каждая смерть несёт информацию: `{cause, last_vitals, nearby_entities,
last_plan, steps_survived}`. Система должна спрашивать после каждого
эпизода: "почему я умер в этот раз?" и обновлять world model / стимулы.

```
# Что происходит сейчас:
episode ends → record avg_len → next episode  (информация выброшена)

# Что должно происходить:
episode ends → post_mortem(death_context) → update world model / stimuli
            → next episode с более богатой моделью
```

Конкретно:
- `cause=health` + `zombie` рядом → усилить aversion к zombies
- `cause=health` + `food=0` при смерти → усилить homeostatic priority еды
- `avg_len=35` (ep15) → что-то сломалось сразу → учиться на коротких эпизодах

**Curiosity переформулируется:**

Не просто "исследуй новое". А "уменьши неопределённость о причинах
смерти". Агент сам формулирует гипотезы: "я умираю от zombies чаще
когда еда низкая — проверю".

**Связь с Принципом 5 (Knowledge flow):**

Принцип 5 описывает поток знания между поколениями через textbook.
Принцип 6 описывает что **именно** должно течь: не просто "что случается
в мире" (facts), но "почему система раньше умирала и как не умирать".

**Текущее состояние:** не реализован. Stage 83 сохраняет experience
между поколениями (SDM persistence), но не анализирует причины смерти
явно. Post-mortem learning — roadmap-level.
