...
for i in range(N_PROFILE):
    ...
    print(f"  cycle {i+1:2d}: total={ct:.0f}ms  engine={engine_times[-1]:.0f}ms  stdp={stdp_times[-1] if stdp_times else '?'}ms  enc={enc_times[-1] if enc_times else '?'}ms", flush=True)
...
print(f"  stdp.apply:    {_avg(stdp_times):6.1f} ms  ({_avg(stdp_times)/_avg(cycle_times)*100:.1f}%)")
