...
**Статус:** Stage 77a (wall ~180) → Stage 78a (FAIL within tested regimes) → 78b/78c partial → 79 partial PASS (Bug 1, 2 fixed) → **Stage 80 partial PASS** (diagnostic bug hunt: 4 new bugs found and fixed including Bug 6 refactored env-agnostic; warmup_a baseline +34, wood +193%, action entropy stabilized; **eval baseline UNCHANGED at ~178** — wall is architectural: planner picks short greedy plans, never crafts tools/sword) → **Decision: B1 score function redesign (default) / B2 hierarchical planning / B3 consolidate**
...
**Правило (2026-04-11):** каждый stage в Roadmap v5 обязан завершаться **Crafter eval со стандартными метриками** — survival mean, wood/coal/iron %, achievements, episode length. Synthetic / unit тесты — только sub-gate внутри stage, не замена. Stage не считается complete без Crafter дельты (положительной или явно нулевой с объяснением). См. `feedback_crafter_every_stage` в auto-memory.
...
|-------|----------|------|-----|
| 78b | MLP Residual Trainer (Dreamer-CDP scaffolding) | Forked from `predictive_trainer.py`: neg cosine similarity loss, stop-gradient target, small MLP residual on top of `simulate_forward` prediction. Synthetic test: учит conjunctive rule за <5 эпох, residual bottleneck kept small | **unit-PASS 2026-04-11** — conj_health_mse=0.0064. **Resolved by Stage 78c partial-FAIL.** Report: `docs/reports/stage-78b-report.md`. |
| 78c | Residual Crafter integration + eval | Вклеить `ResidualBodyPredictor` в `ConceptStore.simulate_forward` + online SGD от env rollouts. Crafter eval ablation off vs on. | **COMPLETE partial FAIL 2026-04-11.** v1 residual_off=169, v2 (Bug 2 fix) residual_off=180.4 (=Stage 77a Run 8). residual_on=152→154 (broken в обоих v1 и v2). Discrimination paradox repro #2. **Stage 78c v1 verdict был Bug-2 contaminated.** Report: `docs/reports/stage-78c-report.md`. |
| 79 | Surprise Accumulator + Rule Nursery (no-LLM) | Per-context surprise bucketing with L1/L2 keying. Template-based candidate emission. Verification → promotion to ConceptStore.learned_rules. Phase 7 в `_apply_tick`. **Gate обязан включать Crafter eval.** | **COMPLETE partial PASS 2026-04-11.** Synthetic conjunctive **PASSED** (no-LLM induction works). Bug 2 (proximity-vs-facing в expand_to_primitive) found and fixed → лифт baseline +14 в eval. v3 nursery_on eval 177.4 ≈ off 177.1 (+0.4) с MAX_ABS_EFFECT=0.5 cap. **Wall не сломан** (still ~180). Stage 79 verdict сменён Stage 80: измерение nursery было против sleeping baseline. Report: `docs/reports/stage-79-report.md`. |
| 80 | Diagnostic bug hunt + planner fixes | 5-ep trace dump, step-by-step diagnostic, фикс багов в planner/spatial_map. | **COMPLETE partial PASS 2026-04-11.** 4 новых bug найдено и зафикшено: **Bug 3** (planner gather plans + has_gain score), **Bug 4** (sim oscillation `dist=0`), **Bug 5** (find_nearest stale player tile), **Bug 6** (spatial_map не очищается после gather, refactored env-agnostic via `store.gatherable_items()`). warmup_a +34, wood +193%, entropy stable. **Eval baseline 177.6 unchanged** — wall архитектурный (lex tuple `(alive, has_gain, min_body, ...)` всегда picks shortest greedy plan, never multi-step crafting chains). Report: `docs/reports/stage-80-report.md`. |
