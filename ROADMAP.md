...
**Статус:** Stage 77a COMPLETE (partial PASS, wall ~180) → Stage 78a COMPLETE (FAIL, DAF residual dropped) → **Stage 78b unit-PASS / Crafter-pending** (synthetic gate met, Crafter eval ещё не прогнан) → Stage 78c NEXT (Crafter integration + eval) → Stage 79 (surprise accumulator + rule nursery, no-LLM) → всё последующее обязано включать Crafter eval в gate
...
|-------|----------|------|-----|
| 78b | MLP Residual Trainer (Dreamer-CDP scaffolding) | Forked from `predictive_trainer.py`: neg cosine similarity loss, stop-gradient target, small MLP residual on top of `simulate_forward` prediction. Synthetic test: учит conjunctive rule за <5 эпох, residual bottleneck kept small | **unit-PASS 2026-04-11** — conj_health_mse=0.0064 (gate ≤0.008), gen_health_mse=0.0002 (gate ≤0.012). 9/9 unit tests. Report: `docs/reports/stage-78b-report.md`. **Crafter-pending:** stage не закрыт до прохождения Stage 78c. |
| 78c | Residual Crafter integration + eval | Вклеить `ResidualBodyPredictor` в `ConceptStore.simulate_forward` как per-tick body-delta correction. Small init → не портит Stage 77a baseline. Online SGD от env rollouts на `gap = actual - rules_pred`. Crafter eval (survival mean, wood/coal/iron %, achievements, episode length). Ablation residual on/off на одинаковых seeds. Gate: ненулевая дельта от Stage 77a baseline (wall=180) или явное объяснение причины нулевой. | 1 нед |
