...
**Статус:** Stage 77a (wall ~180) → Stage 78a (FAIL within tested regimes, 4 methodology gaps) → 78b unit-PASS → 78c partial-FAIL (Bug-2-contaminated) → **Stage 79 partial PASS** (synthetic falsification PASSED, Bug 2 fix лифт baseline +14, MAX_ABS_EFFECT cap → Crafter neutral, wall not broken) → **Decision point: Option A encoding refinement (default) / B strategic pivot / C 78a spikerate retest / D consolidate**
...
| 79 | Surprise Accumulator + Rule Nursery (no-LLM) | Per-context surprise bucketing with L1/L2 keying. Template-based candidate emission. Verification → promotion to ConceptStore.learned_rules. Phase 7 в `_apply_tick`. **Gate обязан включать Crafter eval.** | **COMPLETE partial PASS 2026-04-11.** Synthetic conjunctive **PASSED** (no-LLM induction works). Bug 2 (proximity-vs-facing в expand_to_primitive) found and fixed → лифт baseline +14 в eval. v3 nursery_on eval 177.4 ≈ off 177.1 (+0.4) с MAX_ABS_EFFECT=0.5 cap. **Wall не сломан** (still ~180). Discrimination paradox repro #3 — фундаментальный для всех подходов «learn corrections from prediction error в MPC loop». Report: `docs/reports/stage-79-report.md`. |
