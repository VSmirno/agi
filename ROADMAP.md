...
**Статус:** Stage 77a COMPLETE (partial PASS, wall ~180) → Stage 78a COMPLETE (FAIL within tested regimes, parked Branch C — 4 methodological gaps) → Stage 78b unit-PASS resolved by 78c → **Stage 78c COMPLETE partial FAIL** (eval Δ−17.1 vs off, НО warmup_a +43.6 → wiring correct, encoding too coarse) → Decision point: Stage 79 (rule induction, default) vs Stage 78d (encoding) vs Stage 78a re-test (DAF spikerate)
...
| 78c | Residual Crafter integration + eval | Вклеить `ResidualBodyPredictor` в `ConceptStore.simulate_forward` + online SGD от env rollouts. Crafter eval ablation off vs on. | **COMPLETE partial FAIL 2026-04-11.** residual_off eval 169.2, residual_on eval 152.1 (Δ−17.1). НО residual_on warmup_a 259.5 vs off 215.9 (+43.6 vs +37.5 от Stage 77a baseline 222) — wiring корректен. Action entropy collapse on→0.18 vs off→0.86 в eval. Encoding (visible+body+action, без facing/positions) too coarse для multi-modal corrections. **Bug 1 (planned_step=None в training) найден через systematic-debugging и зафикшен** до full run (`faf3ca3`). Report: `docs/reports/stage-78c-report.md`. |
| 79 | Surprise Accumulator + Rule Nursery (no-LLM) | Per-context surprise bucketing with two-level context key (coarse (visible, action), refined by body quartiles). Template-based candidate rule emission from mean observed delta. Verification gate → promotion to ConceptStore.learned_rules. Sketch: `docs/superpowers/specs/2026-04-11-stage79-surprise-accumulator-sketch.md`. **Gate обязан включать Crafter eval** (не только synthetic), промоутнутые правила двигают Crafter метрики. | 2 нед |
