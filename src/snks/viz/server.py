...
from snks.viz.stage6_data import (
    OUTPUT_PATH as STAGE6_DATA_PATH,
    collect_demo,
    load_data,
    save_data,
)
...
@app.get("/stage6")
async def stage6_page():
    """Serve the Stage 6 experiment visualization page."""
    page_path = STATIC_DIR / "stage6.html"
    if page_path.exists():
        return FileResponse(str(page_path))
    return JSONResponse(
        {"message": "Stage 6 visualization — stage6.html not found"},
        status_code=404,
    )


@app.get("/api/stage6")
async def api_stage6_get():
    """Return Stage 6 JSON data previously generated by /api/stage6/generate."""
    data = load_data(STAGE6_DATA_PATH)
    if data is None:
        raise HTTPException(
            status_code=404,
            detail="stage6_data.json not found. Call POST /api/stage6/generate first.",
        )
    return JSONResponse(content=data)


@app.post("/api/stage6/generate")
async def api_stage6_generate():
    """Generate Stage 6 demo data, save to disk, and return it."""
    loop = asyncio.get_event_loop()
